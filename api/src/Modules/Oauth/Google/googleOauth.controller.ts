...
export class GoogleOAuthController {
  ...
  public static initiateSignUpOrLogin(req: Request, res: Response): void {
    try {
      const url = GoogleOAuthServices.initiateSignUpOrLoginStudentWithGoogle();
      res.status(200).json({ url });
    } catch (error) {
      res
        .status(500)
        .json({
          message: `Error initiating Google sign-up or login: ${error.message}`,
        });
    }
  }
  ...
  public static async handleGoogleCallback(
    ...
    try {
      const data: HandleStudentGoogleLoginCallbackInput = req.body;
      ...
      const result =
        await GoogleOAuthServices.handleStudentGoogleSignUpOrLoginCallback(
          data
        );
      ...
      res
        .status(500)
        .json({ message: `Error handling Google callback: ${error.message}` });
    }
  }
}
